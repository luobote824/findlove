...
function initPhotoUpload() {
    ...
    $("#file-1").fileinput({
        language: 'zh',
        theme: 'fa',
        uploadUrl: contextPath + "usercenter/upload",
    ...
    $('#file-1').on('filebatchuploadsuccess', function(event, data, previewId, index) {
        ...
        if(data.response.result=="true"){
            ...
            if(data.response.result=="porn"){
                swal("温馨提示","图片涉黄，请重新上传！","error");
            }
            else if(data.response.result=="ad"){
                swal("温馨提示","图片违规，请重新上传！","error");
            }
            else  if(data.response.result =="terrorism"){
                swal("温馨提示","图片暴恐涉政，请重新上传！","error");
            }
            else if(data.response.result=="qrcode"){
                swal("温馨提示","图片包含二维码，请重新上传！","error");
            }
            else  if(data.response.result =="live"){
                swal("温馨提示","图片包含不良场景，请重新上传！","error");
            }
            else if(data.response.result=="logo"){
                swal("温馨提示","图片包含logo，请重新上传！","error");
            }
            else if(data.response.result=="logo"){
                swal("温馨提示","图片包含logo，请重新上传！","error");
            }
